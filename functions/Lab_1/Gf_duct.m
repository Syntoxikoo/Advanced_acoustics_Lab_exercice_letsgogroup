    ...
    if length(duct_coord) == 2
        ...
        if isempty(mode_list)
            ...
            % Use specific mode list if provided
                ...
                en = ones(length(mode_list(:,1))) * 2;
                em = ones(length(mode_list(:,1))) * 2;
                ...
                coefx = mode_list(:,1)*pi/lx; 
                coefy = mode_list(:,2)*pi/ly;
        end
        ...
        for ll = 1:length(duct_coord(:,1))
            for jj = 1:length(duct_coord(:,2))
                ...
                for ii = 1:length(f)
                    if ~isempty(mode_list)
                        phim_rM = sqrt(em(ii) .* en(ii)) .* cos(coefx(ii) .* duct_coord(ll,1)) .* cos(coefy(ii) .* duct_coord(jj,2));
                        phim_rS = sqrt(em(ii) .* en(ii)) .* cos(coefx(ii) .* rS(1)) .* cos(coefy(ii) .* rS(2));
                        kzmn = -sqrt(k(ii).^2 - coefx(ii).^2 - coefy(ii).^2);
                    else
                        kzmn = -sqrt(k(ii).^2 - coefx.^2 - coefy.^2);
                    end
